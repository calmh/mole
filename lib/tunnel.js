...
function loadFile(local) {
    if (!existsSync(local)) {
        throw new Error('Tunnel definition "' + local + '" does not exist');
    }
...
function loadIniTunnel(name) {
    ...
    config.hosts = {};
    config.forwards = {};
    config.localForwards = {};
    config.vpnc = obj.vpnc;
    config.vpnRoutes = obj['vpn routes'];
    config.openconnect = obj.openconnect;
    ...
    _.each(obj, function (val, key) {
        ...
        // Forward sections look like [forward A description here]
        m = key.match(/^localforward +(.+)$/);
        if (m) {
            arr = [];
            _.each(val, function (to, from) {
                arr.push({ from: from, to: to });
            });
            config.localForwards[m[1]] = arr;
            return;
        }
    });
