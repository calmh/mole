...
function server(opts, state) {
    ...
    ldapServer = opts.ldapsrv;
    ldapBindDn = opts.ldapdn;
...
function authenticated(func) {
    return function (req, res) {
        ...
        if (!req.headers['x-mole-version'] || !req.headers['x-mole-version'].match(minVersionExp)) {
            res.header('x-error-message', 'Client version unacceptable');
            res.send(530);
            res.end();
            return;
        }

        if (req.headers['x-mole-version'].match(/^3\./)) {
            // v3 client use certificate authentication
            var user = authenticate(req);
            if (user) {
                res.header('x-mole-authenticated', user);
                users.get(user).seen = Date.now();
                users.save();
            }
            req.moleAuthenticated = user;
        } else if (req.headers['x-mole-version'].match(/^4\./)) {
            // v4 clients use ticket authentication
            var tic = req.headers['x-mole-ticket'];
            if (tic) {
                var validTicket = ticket.examine(tic);
                if (validTicket && validTicket.ip === req.connection.socket.remoteAddress) {
                    req.moleAuthenticated = validTicket.user;
                }
            }
        }
...
function startApp(opts) {
    ...
    app.get(/\/users$/, authenticated(audited(listUsers)));
    app.get('/store', authenticated(audited(listfiles)));
    app.get(/\/store\/([0-9a-z_.\-]+\.ini)$/, authenticated(audited(getfile)));
    app.get(/\/pkg/, authenticated(audited(getPkg)));
    app.get(/\/key\/([0-9a-f\-]+)/, authenticated(audited(getKey)));
    app.get('/ping', authenticated(audited(ping)));
    if (opts.ro) {
        app.post('/newtoken', readonly(opts.ro));
        app.post(/\/users\/([a-z0-9_\-]+)$/, readonly(opts.ro));
        app.del(/\/users\/([a-z0-9_\-]+)$/, readonly(opts.ro));
        app.put(/\/store\/([0-9a-z_.\-]+\.ini)$/, readonly(opts.ro));
        app.del(/\/store\/([0-9a-z_.\-]+\.ini)$/, readonly(opts.ro));
    } else {
        ...
        app.put(/\/store\/([0-9a-z_.\-]+\.ini)$/, authenticated(audited(putfile)));
        app.del(/\/store\/([0-9a-z_.\-]+\.ini)$/, authenticated(audited(delfile)));
    }

    app.get(/\/extra\/([0-9a-z_.\-]+)$/, audited(getExtraFile));
    app.post(/\/ticket\/([0-9a-z_.\-]+)$/, audited(newTicket));
...
function readonly(reason) {
    return function (req, res) {
        res.header('x-error-message', 'Server is read only: ' + reason);
        res.send(403);
        res.end();
        return;
    };
...
function ping(req, res) {
    ...
    res.send(200, req.moleAuthenticated);
    res.end();
}
...
function newTicket(req, res) {
    ...
    req.on('end', function () {
        ...
        var dn = ldapBindDn.replace("%s", user);
        var client = ldap.createClient({ url: ldapServer });
        client.bind(dn, password, function (err) {
            if (err) {
                res.send(403, err.message);
            } else {
                var ip = req.connection.socket.remoteAddress;
                var tic = ticket.issue(user, ip, 3600 * 24 * 7);
                res.send(200, tic);
            }
            res.end();
        });
...
function newtoken(req, res) {
    ...
    var user = users.get(req.moleAuthenticated);
    user.token = uuid.v4();
    users.save();
...
function newuser(req, res) {
    ...
    if (users.get(username)) {
        res.header('x-error-message', 'User already exists');
        res.send(409);
        res.end();
        return;
    }
    ...
    function load(fname) {
        ...
        try {
            ...
            return {
                ...
                version: parseFloat(t.general.version || 2.0)
            };
        } catch (e) {
...
function putfile(req, res) {
    ...
    req.on('end', function () {
        ...
        try {
            ...
            audit.error('obfuscate', {msg: e.toString()});
            res.header('x-error-message', e.message);
            res.send(531);
            res.end();
        }
    });
...
function delfile(req, res) {
    ...
    var file = req.params[0];
    con.debug('DELETE /store/' + file);
    fs.unlink(path.join(dataDir, file), function (err) {
        if (err) {
            res.send(404);
        } else {
