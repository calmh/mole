...
function server(opts, state) {
    ...
    server.dlog('Using keys file', keysFile);
...
function startApp(opts) {
    ...
    app.get(/\/key\/([0-9a-f-]+)/, audited(getKey));
    ...
    function stat(fname, callback) {
        ...
        fs.stat(fp, function (err, res) {
            ...
            try {
                ...
                callback(null, { name: fname, error: e.toString() });
            }
        });
    }
