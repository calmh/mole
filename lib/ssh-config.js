...
function forwardConfig(fwds) {
    ...
    _.each(fwds, function (fs, descr) {
        ...
        _.each(fs, function (to, from) {
            lines.push('  LocalForward ' + from + ' ' + to);
        });
    });
    return lines;
}
...
module.exports = function (config, debug) {
    ...
    _.each(config.hosts, function (c, name) {
        ...
        if (c.via) {
            var v = debug ? ' -v' : '';
            lines.push('  ProxyCommand ssh' + v + ' -F ' + config.sshConfig + ' ' + c.via + ' nc -w 1800 %h %p');
        }
        if (c.socks) {
            lines.push('  ProxyCommand nc -x ' + c.socks + ' %h %p');
        }
        ...
        if (c.key) {
            ...
            config.sshKeyFiles.push(keyFile);
