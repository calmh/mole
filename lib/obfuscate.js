...
function obfuscateObj(obj, keyStore) {
    for (var i = 0; i < encryptAttrs.length; i++) {
        ...
        if (obj[attr]) {
            if (obj[attr].indexOf(encryptTag) === 0) {
                var token = obj[attr].substr(encryptTag.length);
                if (!keyStore.get(token)) {
                    throw new Error('Unregistered token ' + token + ' in tunnel configuration.');
                }
            } else {
                var u = uuid.v4();
