...
var defaults = {
    ...
    method: 'GET',
    agent: false,
    headers: {'x-mole-version': pkg.version}
};
...
Server.prototype.unbufferedRequest = function unbufferedRequest(options, callback) {
    var self = this;
    var errors = {
        403: 'Forbidden (invalid certificate or permission denied)',
        404: 'Not Found (resource does not exist or is invalid)',
        530: 'Server requires a newer client version',
        531: 'Unrecognized encryption token (replace with cleartext or valid token)'
    };

    _.defaults(options, defaults);

    con.debug('%s Request; http://%s:%s%s', options.method, options.host, options.port, options.path);
    var req = https.request(options, function (res) {
        ...
        if (res.statusCode !== 200) {
            if (errors[res.statusCode]) {
                con.fatal(res.statusCode + ' ' + errors[res.statusCode]);
            }
            ...
            con.fatal('Server certificate does not match stored fingerprint');
        } else {
            callback(res);
        }
    });
    ...
    req.on('error', function (err) {
        if (err.errno === 'ENOTFOUND') {
            con.fatal('Server "' + options.host + '" not found - please check your DNS and network configuration')
        }
...
// buffers all received data, which is expected to be UTF-8 text. When all data
...
Server.prototype.bufferedRequest = function bufferedRequest(options, callback) {
    var buffer = '';
    return this.unbufferedRequest(options, function (res) {
        var serverCert = res.connection.getPeerCertificate();
        var serverFingerprint = serverCert.fingerprint;
        ...
        res.on('end', function () {
            callback({ buffer: buffer, fingerprint: serverFingerprint, headers: res.headers });
        });
    });
};
...
Server.prototype.list = function list(callback) {
    ...
    this.bufferedRequest({ path: '/store' },function (result) {
        if (result.buffer.length === 0) {
            ...
            callback(JSON.parse(result.buffer), result.headers['x-mole-protocol']);
        }
    }).end();
};
...
Server.prototype.lsUsers = function lsUsers(callback) {
    this.bufferedRequest({ path: '/users' },function (result) {
        callback(JSON.parse(result.buffer));
    }).end();
};
...
Server.prototype.getKey = function getKey(key, callback) {
    this.bufferedRequest({ path: '/key/' + key },function (res) {
        var o = JSON.parse(res.buffer);
        callback(o.key);
    }).end();
};
